...
2. ✅ Added host and join controls, temporary display names, a visible invite code, guest joined-state, and an explicit host end-session control. The join flow is a focused accessible dialog with invite-code formatting, validation feedback, and clear Join/Cancel actions. Invite codes accept typed values with or without the display hyphen and avoid ambiguous `I`, `L`, and `O` characters.
